...
const cinemaPlay  = $('cinemaPlay');
const cinemaMute  = $('cinemaMute');
...
/* ============================================================
   ...
   ============================================================ */
const PHOTOS = [
  { src: '/photos/memory-01.jpg', caption: 'the ones that made me smile first' },
  { src: '/photos/memory-02.jpg', caption: 'a quiet afternoon, made golden' },
  { src: '/photos/memory-03.jpg', caption: 'you, un-posed and lovely' },
  { src: '/photos/memory-04.jpg', caption: 'the day I keep going back to' },
  { src: '/photos/memory-05.jpg', caption: 'small moment, kept forever' },
  { src: '/photos/memory-06.jpg', caption: 'that laugh — right there' },
  { src: '/photos/memory-07.jpg', caption: 'us, on an ordinary good day' },
  { src: '/photos/memory-08.jpg', caption: 'and this one, just because' },
];
...
galleryToFilm.addEventListener('click', () => { closeGallery(); openCinema(); });
...
/* ============================================================
   ACT 6 — THE FILM
   A fullscreen, letterboxed video moment. Opens from the gallery's
   "watch our film" cue (or directly, once opened once).
   ============================================================ */
function openCinema(){
  cinema.hidden = false;
  cinema.setAttribute('aria-hidden', 'false');
  requestAnimationFrame(() => cinema.classList.add('is-open'));
  // best-effort true fullscreen (hides browser chrome where supported);
  // the CSS overlay already covers the full viewport either way.
  const el = cinema;
  const req = el.requestFullscreen || el.webkitRequestFullscreen;
  if (req){ req.call(el).catch(() => {}); }
}
function closeCinema(){
  if (!cinema.classList.contains('is-open')) return;
  cinema.classList.remove('is-open', 'is-playing');
  cinema.setAttribute('aria-hidden', 'true');
  cinemaVideo.pause();
  cinemaMute.hidden = true;
  if (document.fullscreenElement === cinema || document.webkitFullscreenElement === cinema){
...
function toggleCinemaMute(){
  ...
  cinemaMute.querySelector('.ic-off').hidden = !cinemaVideo.muted;
  cinemaMute.querySelector('.ic-on').hidden = cinemaVideo.muted;
  cinemaMute.setAttribute('aria-label', cinemaVideo.muted ? 'Turn sound on' : 'Mute');
}

cinemaClose.addEventListener('click', closeCinema);
cinemaPlay.addEventListener('click', toggleCinemaPlay);
cinemaVideo.addEventListener('click', toggleCinemaPlay);
cinemaMute.addEventListener('click', (e) => { e.stopPropagation(); toggleCinemaMute(); });

document.addEventListener('keydown', (e) => {
  if (e.key === 'Escape'){
    if (cinema.classList.contains('is-open')) closeCinema();
    else if (gallery.classList.contains('is-open')) closeGallery();
  }
  if (gallery.classList.contains('is-open')){
    if (e.key === 'ArrowRight') showSlide(gIndex + 1, true);
    if (e.key === 'ArrowLeft')  showSlide(gIndex - 1, true);
  }
});

/* pause the slideshow (not the film) when the tab is hidden */
